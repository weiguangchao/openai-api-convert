...
import { startBridge, type Upstream } from './server.ts';
...
export const runReleaseSmoke = async ({ apiKey, upstreams, model = 'gpt-4.1', codexBin = 'codex', runCodex = runCodexCli }: ReleaseSmokeOptions) => {
  ...
  try {
    ...
    const response = await fetch(`${bridge.url}/v1/responses`, {
      ...
      body: JSON.stringify({ model, stream: true, input: 'Reply with a short acknowledgement.' }),
    });
    ...
    const args = [
      codexBin, 'exec', '--ephemeral', '--ignore-user-config', '--skip-git-repo-check', '--disable', 'multi_agent', '--disable', 'apps', '-c', 'web_search="disabled"', '-C', dir,
