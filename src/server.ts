...
type BridgeLogger = { log: BridgeLog; level: LogLevel; close: () => Promise<void> };
type RequestContext = { options: BridgeOptions; state: StateStore; logging: BridgeLogger; metrics: Metrics };
type ResponseScope = { responseId: string | undefined };
...
type BuiltRequest = { upstreamBody: Record<string, unknown>; namespaceAliases: ReturnType<typeof buildNamespaceAliasMaps>; model: string };
type ClaimResult = { kind: 'reused' } | { kind: 'created'; responseId: string };
type StreamUpstreamParams = { responseId: string; model: string; upstreamBody: Record<string, unknown>; namespaceAliases: ReturnType<typeof buildNamespaceAliasMaps>; upstreams: Upstream[] };
type CancelState = { cancelled: boolean; activeAbort: AbortController | undefined };
type AttemptCounter = { value: number };
type AttemptUpstreamParams = {
  upstream: Upstream; id: string; model: string; upstreamBody: Record<string, unknown>; namespaceAliases: ReturnType<typeof buildNamespaceAliasMaps>;
...
const handleResponsesRequest = async (ctx: RequestContext, request: IncomingMessage, response: ServerResponse, requestId: string, scope: ResponseScope) => {
  const parsed = await parseAndValidateRequest(ctx, request, response, requestId);
  if (!parsed) return;
  const resolved = resolveChainAndCapabilities(ctx, response, parsed.payload, parsed.input, parsed.tools);
  if (!resolved) return;
  const built = buildChatRequest(response, parsed.payload, resolved.effectiveTools, resolved.ancestors, parsed.input, resolved.degradeWebSearch, parsed.reasoningEffort);
  if (!built) return;
  const claimed = await claimOrCreateResponse(ctx, request, response, parsed.payload, parsed.input, parsed.tools, parsed.reasoningEffort, parsed.idempotencyKey, built.model, scope);
  if (!claimed) return;
  if (claimed.kind === 'reused') return;
  await streamUpstreamAttempts(ctx, request, response, requestId, { responseId: claimed.responseId, model: built.model, upstreamBody: built.upstreamBody, namespaceAliases: built.namespaceAliases, upstreams: resolved.upstreams });
};

const parseAndValidateRequest = async (ctx: RequestContext, request: IncomingMessage, response: ServerResponse, requestId: string): Promise<ParsedRequest | null> => {
  const { options, logging } = ctx;
  if (!requireBridgeAuthentication(request, response, options.apiKey)) return null;
  ...
  catch { sendError(response, 400, 'Invalid JSON body', 'invalid_json'); return null; }
  ...
  return { payload, rawBody, idempotencyKey, reasoningEffort, input, tools };
};

const resolveChainAndCapabilities = (ctx: RequestContext, response: ServerResponse, payload: ResponsesPayload, input: InputItem[], tools: Tool[] | undefined): ResolvedChain | null => {
  ...
  if (payload.previous_response_id) {
    ...
    catch { sendError(response, 400, 'Previous response was not found', 'previous_response_not_found'); return null; }
  }
  ...
  return { ancestors, effectiveTools, upstreams, degradeWebSearch, needs };
};

const buildChatRequest = (response: ServerResponse, payload: ResponsesPayload, effectiveTools: Tool[], ancestors: StoredResponse[], input: InputItem[], degradeWebSearch: boolean, reasoningEffort: string | undefined): BuiltRequest | null => {
  ...
  return { upstreamBody, namespaceAliases, model };
};

const claimOrCreateResponse = async (ctx: RequestContext, request: IncomingMessage, response: ServerResponse, payload: ResponsesPayload, input: InputItem[], tools: Tool[] | undefined, reasoningEffort: string | undefined, idempotencyKey: string | undefined, model: string, scope: ResponseScope): Promise<ClaimResult | null> => {
  const { state } = ctx;
  ...
  if (claim.kind === 'reused') {
    scope.responseId = claim.responseId;
    response.writeHead(200, { 'content-type': 'text/event-stream', 'cache-control': 'no-cache', connection: 'keep-alive' });
    await replaySse(response, state, claim.responseId);
    return { kind: 'reused' };
  }
  scope.responseId = id;
  return { kind: 'created', responseId: id };
};

const attemptUpstream = async (ctx: RequestContext, response: ServerResponse, requestId: string, params: AttemptUpstreamParams): Promise<AttemptOutcome> => {
  const { options, state, logging, metrics } = ctx;
  ...
  try {
    ...
    finishAttempt();
    if (cancelState.cancelled) {
      return { outcome: 'terminal', attempt: { id: attemptId, result: 'cancelled', preOutputFailure: !outputStarted, errorCode: 'client_disconnected' }, failedOutputText: outputText, streamStarted };
    }
    if (outputStarted) {
      return { outcome: 'terminal', attempt: { id: attemptId, result: 'failed', preOutputFailure: false, errorCode: 'upstream_stream_failed' }, failedOutputText: outputText, streamStarted };
    }
    return { outcome: 'retryable', attempt: { id: attemptId, result: 'failed', preOutputFailure: true, errorCode: 'upstream_retryable' }, streamStarted };
  }
};

const streamUpstreamAttempts = async (ctx: RequestContext, request: IncomingMessage, response: ServerResponse, requestId: string, params: StreamUpstreamParams) => {
  ...
  const cancel = () => { errorCodes.set(response, 'client_disconnected'); cancelState.cancelled = true; cancelState.activeAbort?.abort(); };
